...
abstract class TreeBaseContainer<K, V> extends Base {
  ...
  protected findSubTreeMinNode:
  (curNode: TreeNode<K, V>) => TreeNode<K, V> =
      (curNode: TreeNode<K, V>) => {
        return curNode.leftChild ? this.findSubTreeMinNode(curNode.leftChild) : curNode;
      };
  protected findSubTreeMaxNode:
  (curNode: TreeNode<K, V>) => TreeNode<K, V> =
      (curNode: TreeNode<K, V>) => {
  ...
  protected eraseNodeSelfBalance(curNode: TreeNode<K, V>) {
    ...
    if (!parentNode || parentNode === this.header) return;
  ...
  protected eraseNode(curNode: TreeNode<K, V>) {
    ...
    if (
      ...
      this.cmp(this.header.leftChild.key, swapNode.key as K) === 0
    ) {
    ...
    if (this.header.rightChild &&
      ...
      this.cmp(this.header.rightChild.key, swapNode.key as K) === 0
    ) {
