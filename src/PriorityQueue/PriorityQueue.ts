...
class PriorityQueue<T> implements Base {
    ...
    private swap(x: number, y: number) {
        if (x < 0 || x >= this.length) throw new Error("unknown error");
        if (y < 0 || y >= this.length) throw new Error("unknown error");
    ...
    private adjust(parent: number) {
        if (parent < 0 || parent >= this.length) throw new Error("unknown error");
    ...
    pop() {
        ...
        while (parent < this.size()) {
            ...
            let minChild = leftChild;
            if (rightChild < this.size() && this.cmp(this.priorityQueue[leftChild], this.priorityQueue[rightChild]) > 0) minChild = rightChild;
     ...
     */
    top() { return this.priorityQueue[0]; }
}
