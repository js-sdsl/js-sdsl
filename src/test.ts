...
import { ContainerIterator, SequentialContainer, Pair } from './types/interface';
  ...
  function judgeStack(myStack: Stack<number>, myVector: Vector<number>) {
    while (!myStack.empty()) {
      if (myStack.size() !== myVector.size()) {
        throw new TestError('Stack size test failed!');
      }
      const s = myStack.top();
      const v = myVector.back();
      if (s !== v) {
        throw new TestError('Stack test failed!');
      }
      myStack.pop();
      myVector.popBack();
    }
...
function judgeSequentialContainer(funcName: string, container: SequentialContainer<number>, myVector: SequentialContainer<number>) {
  ...
  if (!testResult) throw new TestError(`${funcName} test failed!`);
...
function testPriorityQueue(testNum: number) {
  ...
  while (!myPriority.empty()) {
    if (myPriority.size() !== myVector.size()) {
      throw new TestError('PriorityQueue size test failed!');
    }
    const u = myPriority.top();
    if (u !== myVector.front()) {
      throw new TestError('PriorityQueue test failed!');
    }
    myPriority.pop();
    myVector.eraseElementByPos(0);
  }
...
function testOrderedSet(testNum: number) {
  function judgeSet(myOrderedSet: OrderedSet<number>, myVector: Vector<number>) {
    if (myOrderedSet.getHeight() > 2 * Math.log2(myOrderedSet.size() + 1)) {
      throw new TestError('OrderedMap tree too high!');
    }
    if (myOrderedSet.size() !== myVector.size()) {
      throw new TestError('OrderedMap size test failed!');
    }
  ...
  for (let i = 0; i < myVector.size(); ++i) {
    if (myOrderedSet.reverseLowerBound(myVector.getElementByPos(i)).pointer !== myVector.getElementByPos(i)) {
      throw new TestError('OrderedMap reverseLowerBound test failed!');
    }
    if (i !== 0 && myOrderedSet.reverseUpperBound(myVector.getElementByPos(i)).pointer !== myVector.getElementByPos(i - 1)) {
      throw new TestError('OrderedMap reverseUpperBound test failed!');
    }
  }
...
function testHashSet(testNum: number) {
  function judgeHashSet(funcName: string, myHashSet: HashSet<number>, stdSet: Set<number>) {
    if (myHashSet.size() !== stdSet.size) {
      throw new TestError(`HashSet ${funcName} test failed!`);
    }
...
function testHashMap(testNum: number) {
  function judgeHashMap(funcName: string, myHashMap: HashMap<number, number>, stdMap: Map<number, number>) {
    if (myHashMap.size() !== stdMap.size) {
      throw new TestError(`HashMap ${funcName} test failed!`);
    }
    stdMap.forEach((value, key) => {
...
function testIterator() {
  ...
  for (const container of containerArr) {
    ...
    for (let it = container.begin() as ContainerIterator<unknown>; !it.equals(container.end() as ContainerIterator<unknown>); it = it.next()) {
      ...
      if (!testPassed) {
        throw new TestError(`${container.constructor.name}'s normal iterator TestError!`);
      }
    }
  }
  ...
  for (const container of containerArr) {
    ...
    for (let it = container.rBegin() as ContainerIterator<unknown>; !it.equals(container.rEnd() as ContainerIterator<unknown>); it = it.next()) {
      ...
      if (!testPassed) throw new TestError(`${container.constructor.name}'s reverse iterator TestError!`);
    }
  }
